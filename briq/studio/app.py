"""Minimal bundled briq Studio — FastAPI backend + inline HTML dashboard.

No external briq_studio package required. Works with: pip install briq[studio]
"""
...
<body>
  ...
  <main id="main">
    <div id="topbar">
      ...
      <button class="btn primary" onclick="triggerRun()">▶ briq run</button>
      <button class="btn" onclick="triggerPlan()">◎ briq plan</button>
  ...
  <script>
    ...
    async function loadData() {
      try {
        ...
        _data = await r.json();
        document.getElementById('proj-name').textContent = _data.name || 'briq project';
        renderCurrent();
      } catch(e) {
    ...
    function renderOverview() {
      ...
      document.getElementById('page-overview').innerHTML = `
        ...
        <div class="card">
          ...
          `).join('') || '<div class="empty">No runs yet — click ▶ briq run to start</div>'}
        </div>`;
    }
...
def create_app(project_dir: str = ".") -> FastAPI:
    """Create the briq Studio FastAPI application."""
    app = FastAPI(title="briq Studio", docs_url=None, redoc_url=None)
    ...
    async def run_project():
        try:
            result = subprocess.run(
                [sys.executable, "-m", "briq", "run", "--project-dir", str(_project_dir)],
                capture_output=True,
                text=True,
                timeout=300,
            )
    ...
    async def plan_project():
        try:
            result = subprocess.run(
                [sys.executable, "-m", "briq", "plan", "--project-dir", str(_project_dir)],
                capture_output=True,
                text=True,
                timeout=30,
            )
