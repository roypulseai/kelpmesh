...
from briq.core.crypto import encrypt_file, decrypt_file, is_encrypted
...
def _open_state(db_path: Path, encryption_key: str | None = None, read_only: bool = False) -> tuple[duckdb.DuckDBPyConnection, Path | None]:
    ...
    use_encryption = bool(encryption_key or os.environ.get("BRIQ_ENCRYPTION_KEY"))
    tmp_path = None
    ...
    if use_encryption and db_path.exists():
        ...
        if is_encrypted(raw):
            ...
            if decrypted is None:
                raise RuntimeError("Failed to decrypt state database. Check BRIQ_ENCRYPTION_KEY.")
...
class StateEngine:
    ...
    def __init__(self, project_path: Path, encryption_key: str | None = None):
        self.db_path = project_path / "target" / "briq_state.duckdb"
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        self._lock = threading.Lock()
        self._use_encryption = bool(encryption_key or os.environ.get("BRIQ_ENCRYPTION_KEY"))
        self._tmp_path = None
    ...
    def open_readonly(cls, db_path: Path, encryption_key: str | None = None) -> "StateEngine":
        ...
        engine._use_encryption = bool(encryption_key or os.environ.get("BRIQ_ENCRYPTION_KEY"))
