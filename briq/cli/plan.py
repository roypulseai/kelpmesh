...
Unlike any feature in dbt Core or SQLmesh, briq plan gives you a Terraform-style
preview: every model, its current state, what action briq would take, and how many
...
def plan_cmd(
    ...
    Shows each model, the action briq would take (RUN / SKIP / NEW / STALE),
    ...
    Examples:
        ...
        briq plan --tag finance --var start_date=2025-01-01
    """
    ...
    console.print(f"\n[bold]briq plan[/bold]  [dim]{project.path.name}[/dim]{env_label}{var_label}\n")
    ...
    console.print()
    console.print("  [dim]Run [bold]briq run[/bold] to execute.[/dim]\n")
