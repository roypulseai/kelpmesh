"""briq security CLI — audit, classify, masking, RLS, erasure commands."""
...
from briq.security.classifier import DataClassifier
...
from briq.security.rls import RlsEngine
from briq.security.erasure import erase_pii
...
def classify_cmd(
    ...
    from briq.security.classifier import DEFAULT_RULES
    if json_out:
...
def rls_cmd(
    ...
    if not policies:
        ...
        console.print("  Create security.yml or add rls: section to briq.yml")
        return
...
def security_status_cmd(
    ...
    rls_in_briq = False
    briq_yml = project_path / "briq.yml"
    if briq_yml.exists():
        import yaml
