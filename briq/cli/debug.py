"""briq debug — validate project config, connection, and environment."""
...
_FIELD_HINTS: dict[str, dict[str, str]] = {
    "postgres": {
        "host": "Set 'host' in briq.yml warehouse config",
        "user": "Set 'user' in briq.yml warehouse config",
        "password": "Set 'password' (or use BRIQ_PG_PASSWORD env var)",
        "database": "Set 'database' in briq.yml warehouse config",
    },
    "redshift": {
        "host": "Set 'host' to your Redshift cluster endpoint",
        "user": "Set 'user' in briq.yml warehouse config",
        "password": "Set 'password' (or use BRIQ_RS_PASSWORD env var)",
        "database": "Set 'database' in briq.yml warehouse config",
    },
    "snowflake": {
        "account": "Set 'account' to your Snowflake account identifier (e.g. xy12345.us-east-1)",
        "user": "Set 'user' in briq.yml warehouse config",
        "password": "Set 'password' (or use BRIQ_SF_PASSWORD env var)",
    },
    "bigquery": {
...
def _check_config(config) -> list[tuple[str, str, str]]:
    ...
    for field in check_fields:
        ...
        if val:
            ...
            hint = hints.get(field, f"Set '{field}' in briq.yml")
...
def debug_cmd(
    ...
    Checks briq.yml fields, tests the warehouse connection, reports
    model count, state, encryption, and telemetry guard status.

    Examples:
        briq debug
        briq debug --no-connection   # skip live connection test
    """
    ...
    console.print()
    console.print(Panel(f"[bold]briq debug[/bold]  [dim]{project_path}[/dim]", expand=False))
    console.print()
    ...
    try:
        ...
        console.print("\n  [red]Fix briq.yml before proceeding.[/red]\n")
        raise typer.Exit(1)
    ...
    if not all_ok:
        console.print(
            "\n  [yellow]Fix missing fields in briq.yml before running.[/yellow]\n"
            "  Example briq.yml:\n"
            "  [dim]warehouse:\n"
            "    type: postgres\n"
            "    host: localhost\n"
    ...
    if connection:
        ...
        try:
            ...
            if "password authentication failed" in msg_lower or "authentication failed" in msg_lower:
                console.print("          [dim]→ Check 'password' in briq.yml (wrong credentials)[/dim]")
                ...
                console.print("          [dim]→ SSL error — try adding sslmode: require to briq.yml[/dim]")
    ...
    if enc_key:
        ...
        console.print("  [dim]·[/dim]  encryption key     Not set (export BRIQ_ENCRYPTION_KEY=<key>)")
    ...
    try:
        ...
        briq_ver = version("briq")
    except Exception:
        briq_ver = "dev"
    py_ver = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    console.print(f"  [dim]briq {briq_ver} · Python {py_ver} · {sys.platform}[/dim]")
    console.print()
