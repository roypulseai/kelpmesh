"""briq mesh — cross-project mesh commands."""
...
def _load_mesh(workspace: Path):
    from briq.mesh.config import MeshConfig
    cfg = MeshConfig.load(workspace)
    if cfg.is_empty():
        err_console.print(
            "[red]No mesh.yml found.[/red] Create one with [bold]briq mesh init[/bold]."
        )
        raise typer.Exit(1)
    return cfg
...
def mesh_init(
    ...
    console.print("Edit it to list your real projects, then run [bold]briq mesh status[/bold].")
...
def mesh_status(
    ...
    from briq.mesh.health import MeshHealthChecker
    ...
    if report.all_healthy:
        ...
        console.print(
            ...
            "Run [bold]briq mesh validate[/bold] for details."
        )
...
def mesh_publish(
    ...
    import datetime
    from briq.mesh.contracts import ProducerContract, InterfaceModel, InterfaceColumn
    from briq.mesh.access import AccessChecker
    from briq.mesh.config import MeshConfig
