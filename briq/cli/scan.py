"""briq scan — secrets scanner for hardcoded credentials in SQL files."""
...
IGNORE_COMMENT = re.compile(r"--\s*briq:scan-ignore\b")
...
def scan_secrets(
    ...
    typer.echo(
        ...
        f"briq.yml for secrets. Mark false positives with "
        f"[bold]-- briq:scan-ignore[/bold] comment."
    )
...
def generate_key():
    """Generate a random encryption key for BRIQ_ENCRYPTION_KEY."""
    try:
        from cryptography.fernet import Fernet
        key = Fernet.generate_key().decode()
        typer.echo(f"Encryption key:\n  {key}")
        typer.echo("\nSet it as an environment variable:")
        typer.echo(f"  export BRIQ_ENCRYPTION_KEY={key}  # Linux/macOS")
        typer.echo(f'  set BRIQ_ENCRYPTION_KEY={key}    # Windows cmd')
        typer.echo(f'  $env:BRIQ_ENCRYPTION_KEY="{key}" # PowerShell')
    except ImportError:
        typer.echo("Install cryptography: pip install briq[studio]")
        raise typer.Exit(1)
