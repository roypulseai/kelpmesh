"""briq compile — render all template expressions and write compiled SQL to target/compiled/.

Unlike `briq run`, this command never touches the warehouse.  It lets you
inspect exactly what SQL briq will execute, including:
...
from briq.core.substitutions import apply as apply_substitutions, parse_cli_vars
...
def compile_cmd(
    ...
    Examples:
        briq compile                      # compile all models
        briq compile orders_daily         # compile one model
        briq compile --select +orders     # compile orders and its upstream deps
        briq compile --var start=2025-01  # with variable override
        briq compile --print orders       # print to stdout
    """
    ...
    if print_sql:
        ...
        console.print(f"\n[bold]briq compile[/bold]  [dim]{project.path.name}[/dim]")
