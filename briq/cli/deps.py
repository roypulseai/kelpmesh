...
def deps_cmd(
    ...
    if action == "add":
        if not package_name:
            console.print("[red]Usage: briq deps add <package> [--source <path>] [--version <ver>][/red]")
            raise typer.Exit(1)
        info = package_info(package_name)
        if info and not source:
            source = info["name"]
        add_package(project_path, package_name, source, version)
        console.print(f"[green]Added package '{package_name}' to briq.lock[/green]")
        console.print("Run [bold]briq deps install[/bold] to install.")

    elif action == "remove":
        if not package_name:
            console.print("[red]Usage: briq deps remove <package>[/red]")
            raise typer.Exit(1)
        ...
        packages = list_packages(project_path)
        if not packages:
            console.print("[yellow]No packages in briq.lock[/yellow]")
            return
        table = Table(title="briq packages")
        ...
        if not package_name:
            console.print("[red]Usage: briq deps info <package>[/red]")
            raise typer.Exit(1)
        info = package_info(package_name)
        if info:
