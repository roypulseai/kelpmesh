...
def import_cmd(
    ...
    output_dir: Path = typer.Option(
        ".", "--output", "-o", help="Output directory for briq project"
    ),
):
            ...
            if pkg_data and "packages" in pkg_data:
                ...
                for pkg in packages:
                    if isinstance(pkg, dict):
                        ...
                        console.print(f"    - {name} (no briq equivalent yet)")
                console.print("  [yellow]Consider replacing with briq-utils or SQL macros.[/yellow]")
