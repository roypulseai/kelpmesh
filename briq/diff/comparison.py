"""Compare briq model output against dbt output row-by-row."""
import logging
        ...
        if isinstance(result["briq_row_count"], int) and isinstance(result["dbt_row_count"], int):
            ...
            if not result["match"]:
                result["differences"].append(
                    f"Row count mismatch: briq={result['briq_row_count']}, dbt={result['dbt_row_count']}"
                )
                try:
                    ...
                    if only_in_briq:
                        result["differences"].append(f"Rows only in briq: {only_in_briq}")
                    if only_in_dbt:
