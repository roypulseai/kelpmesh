"""Package manager: resolve, download, and install kelpmesh packages."""
import json
import shutil
import subprocess
...
KNOWN_REGISTRY = {
    "kelpmesh-utils": {
        "description": "SQL utility macros and helpers for kelpmesh",
        "version": "0.1.0",
        "git": "https://github.com/kelpmesh-dev/kelpmesh-utils.git",
    },
    "kelpmesh-expectations": {
        "description": "Data quality test templates for kelpmesh",
        "version": "0.1.0",
        "git": "https://github.com/kelpmesh-dev/kelpmesh-expectations.git",
    },
}


def _packages_dir(project_path: Path) -> Path:
    return project_path / "kelpmesh_packages"


def _lock_path(project_path: Path) -> Path:
    return project_path / "kelpmesh.lock"
...
def install_packages(project_path: Path):
    """Install all packages listed in kelpmesh.lock into kelpmesh_packages/."""
    lock = _load_lock(project_path)
...
def package_info(name: str) -> Optional[dict]:
    if name in KNOWN_REGISTRY:
        ...
        return {
            ...
            "description": info["description"],
            "version": info["version"],
            "source": info.get("git", f"https://github.com/kelpmesh-dev/{name}"),
        }
    return None
...
def _install_from_registry(name: str, dest: Path, registry_info: Optional[dict] = None):
    ...
    if not readme.exists():
        readme.write_text(f"# {name}\n\nSee https://github.com/kelpmesh-dev/{name}\n", encoding="utf-8")
