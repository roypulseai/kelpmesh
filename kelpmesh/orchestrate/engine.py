...
class Orchestrator:
    """Orchestrates runs across multiple kelpmesh projects respecting cross-project dependencies.

    Projects declare upstream project dependencies in kelpmesh.yml:
        depends_on:
          - upstream_project
    """
    ...
    def _load_projects(self):
        for child in sorted(self.projects_dir.iterdir()):
            if child.is_dir() and (child / "kelpmesh.yml").exists():
                try:
