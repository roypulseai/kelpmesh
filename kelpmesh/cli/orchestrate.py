...
def orchestrate_cmd(
    project_dir: Path = typer.Option(".", "--project-dir", "-p", help="Directory containing kelpmesh projects"),
    ...
    """Run multiple kelpmesh projects in dependency order."""
    orch = Orchestrator(project_dir)
    order = orch.execution_order()

    if not order:
        console.print("[yellow]No kelpmesh projects found in directory.[/yellow]")
        raise typer.Exit(1)
