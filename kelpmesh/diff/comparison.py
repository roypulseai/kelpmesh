...
class ComparisonEngine:
    ...
    def compare(self, model_name: str) -> dict:
        ...
        try:
            result["kelpmesh_row_count"] = self.kelpmesh_adapter.fetch_row_count(table_name)
        except Exception as e:
            result["kelpmesh_row_count"] = f"error: {e}"
        ...
        if isinstance(result["kelpmesh_row_count"], int) and isinstance(result["dbt_row_count"], int):
            result["match"] = result["kelpmesh_row_count"] == result["dbt_row_count"]
            if not result["match"]:
                result["differences"].append(
                    f"Row count mismatch: kelpmesh={result['kelpmesh_row_count']}, dbt={result['dbt_row_count']}"
                )
                try:
                    kelpmesh_data = self.kelpmesh_adapter.execute(f"SELECT * FROM {table_name} ORDER BY 1")
                    dbt_data = self.dbt_adapter.execute(f"SELECT * FROM {table_name} ORDER BY 1")
                    kelpmesh_set = {tuple(d.values()) for d in kelpmesh_data}
                    dbt_set = {tuple(d.values()) for d in dbt_data}
                    only_in_kelpmesh = len(kelpmesh_set - dbt_set)
                    only_in_dbt = len(dbt_set - kelpmesh_set)
                    if only_in_kelpmesh:
                        result["differences"].append(f"Rows only in kelpmesh: {only_in_kelpmesh}")
                    if only_in_dbt:
