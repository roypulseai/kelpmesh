# briq — Project Status & Strategy

> *"If you can write SQL, you can use briq. No engineering degree required."*

---

## Table of Contents

1. [What is briq?](#what-is-briq)
2. [Why Was Jinja Created?](#why-was-jinja-created)
3. [Can We Fully Eliminate Jinja?](#can-we-fully-eliminate-jinja)
4. [Can briq Beat dbt?](#can-briq-beat-dbt)
...
**briq** is an open-core SQL transformation and data modelling platform that lets anyone who writes SQL build reliable, tested, documented, and version-controlled data models — without learning Jinja templating, Git, or software engineering workflows.
...
User writes pure SQL
        ↓
briq Python backend parses the AST
...
| Problem | Jinja Solution (dbt) | sqlglot Solution (briq) |
|---------|---------------------|------------------------|
...
| Programmability | Jinja macros/loops | Python plugin system or `briq generate` CLI |
...
| Jinja Feature | briq Replacement | Status |
|---------------|-----------------|--------|
| `{{ ref('model') }}` | Auto-resolved via sqlglot AST | ✅ Done |
| `{{ source('src', 'table') }}` | Auto-resolved as plain table ref | ✅ Done |
| `{{ config(...) }}` | File headers `-- { materialized: table }` or `briq.yml` | ✅ Done |
| `{{ this }}` | Self-references handled by model name | ✅ Done |
| Jinja macros | Python plugin system (`briq_plugins/` directory) | 🔲 Not started |
| `{{ dbt_utils.* }}` | `briq-utils` Python package with same patterns | 🔲 Not started |
| Jinja loops (`{% for %}`) | Config-driven generators (`briq generate staging --from source`) | 🔲 Not started |
...
1. **Generating repetitive SQL** (e.g., staging models for 20 source tables) → `briq generate staging --schema raw`
...
### The bull case for briq

**1. Addressable market is 5-10x larger**

dbt targets ~500K analytics engineers. briq targets **everyone who writes SQL**:
...
dbt has tried and failed to build a good VS Code extension because Jinja breaks the editor. briq's VS Code extension ships with:
- Full intellisense
...
dbt charges extra (Cloud Enterprise) for column-level lineage. briq ships it free in Core.

### The risks

**1. Community network effects**

dbt has 9 years of community, 5,000+ packages, 30,000+ GitHub stars, and thousands of blog posts/tutorials. briq starts at zero.

**Strategy:** Nail the dbt migration story (`briq import --from dbt`), build core utility packages fast, and invest in community from day 1.
...
dbt has 9 years of feature development. briq has weeks. Users will compare feature lists and see gaps.
...
**briq can beat dbt** — but not by being a better dbt. It wins by serving a different, larger market (finance/ops/business analysts) that dbt never designed for, while being good enough for analytics engineers who are tired of Jinja.

The threat to dbt is not that briq is a better analytics engineering tool — it's that briq makes analytics engineering **irrelevant** by letting business users own their own data models.
...
|---------|-------------|--------|
| `briq init` | Scaffold new project with folder structure | ✅ |
| `briq run` | Execute models in dependency order | ✅ |
| `briq test` | Run SQL assertion tests | ✅ |
| `briq build` | Run + test in one command | ✅ |
| `briq diff <model>` | Compare current vs previous run output | ✅ |
| `briq docs` | Generate static HTML documentation site | ✅ |
| `briq import --from dbt` | Migrate dbt project (strips Jinja, converts refs) | ✅ |
| `briq seed` | Load seed SQL data | ✅ |
...
| `{{ this }}` → removed | ✅ |
| YAML model config → briq.yml | ✅ |
| SQL test files → briq test files | ✅ |
...
| Model | Description |
|-------|-------------|
| `raw_customers` / `raw_orders` | Seed tables (loaded via `briq seed`) |
...
|---------|----------|-------|
| briq Studio (browser UI) | High | React + Monaco editor backend |
| briq Cloud (managed infra) | Low | Year 2 target |
...
| `briq-utils` package | Medium | dbt-utils equivalent |
| `briq-expectations` | Low | dbt-expectations equivalent |
...
| **`briq preview` CLI** | Adapter.preview exists, no CLI wrapper | 2 hours |
| **Column-level lineage in docs** | LineageExplorer class exists, not wired into docs generator | 1 day |
| **Schema drift detection** | VS Code extension has basic check, no `briq schema` command | 2 days |
...
| `briq diff` sample rows | Sample diff query is a placeholder, not true EXCEPT comparison | Reports row count deltas correctly; sample rows unreliable |
...
| 2 | **`briq preview` CLI** | 🟡 | ⏱ 2 hours | Wire `adapter.preview(sql, limit=100)` into a CLI command. Simple but high-visibility feature. |
| 3 | **Schema drift detection** | 🟡 | 📅 2 days | `briq schema diff` compares upstream table schemas against stored state. Alerts when columns change. |
...
| 5 | **CI/CD templates** | 🔴 | ⏱ 4 hours | GitHub Action + GitLab CI template that runs `briq build` on PR. Blocking for team adoption. |
...
| 10 | **`briq-utils` package** | 🟡 | 📅 3 days | `spine`, `date_spine`, `surrogate_key`, `group_by` replacements. `pip install briq-utils`. |
| 11 | **`briq generate`** | 🟢 | 📅 3 days | `briq generate staging --from raw_orders` — scaffolds staging models from source table schemas. |
| 12 | **VS Code extension publish** | 🟡 | 📅 2 days | Package as `.vsix`, test on clean VS Code, publish to marketplace. |
| 13 | **Open source launch** | 🔴 | 📅 3 days | GitHub repo, Apache 2.0 license, `briq.dev`, docs site, HN/Reddit posts, demo video. |

### Phase C — briq Studio (weeks 7-12)
...
```
briq today:       Working CLI POC/MVP — 8 commands, 25 tests, end-to-end verified
briq in 3 weeks:  Production-ready CLI — parallel, CI/CD, packaged on PyPI
briq in 6 weeks:  Open source launch — community, packages, VS Code marketplace
briq in 12 weeks: Studio beta — browser UI for finance/ops/analyst users
briq in 6 months: Cloud launch — managed infrastructure, enterprise features
```
