"""BriqOperator — execute briq commands as Airflow tasks."""
...
class BriqOperator(BaseOperator):
    """Execute a briq command (run, build, test) as an Airflow task.

    Args:
        briq_cmd: briq command to run ('run', 'build', 'test').
        project_dir: Path to the briq project directory.
    ...
    def execute(self, context):
        ...
        adapter.disconnect()
        state.close()
        self.log.info("briq %s completed successfully", self.briq_cmd)
