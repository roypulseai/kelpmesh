"""Alert channels — Slack, email, and generic webhooks for briq Studio Pro."""
...
class AlertDispatcher:
    ...
    def test_channel(self, channel_id: int, org_id: str) -> dict:
        ...
        cfg = json.loads(ch.config)
        ok, detail = self._send(ch.channel_type, cfg, "briq test alert", "This is a test alert from briq Studio.", {})
    ...
    def _send_email(self, cfg: dict, title: str, message: str) -> tuple[bool, str]:
        ...
        msg = EmailMessage()
        msg["Subject"] = f"[briq] {title}"
        msg["From"] = cfg.get("from_address", "noreply@briq.dev")
